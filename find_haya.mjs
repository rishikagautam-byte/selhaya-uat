const store = 'b15301-8f.myshopify.com';
...
console.log('\n=== Looking for HAYA-related ===');
products.filter(p => 
  p.node.handle.toLowerCase().includes('haya') ||
  p.node.handle.toLowerCase().includes('malika') ||
  p.node.handle.toLowerCase().includes('noor') ||
  p.node.handle.toLowerCase().includes('sharifa')
).forEach(({ node }) => {
  console.log(`Found: ${node.handle} - ${node.title}`);
});
