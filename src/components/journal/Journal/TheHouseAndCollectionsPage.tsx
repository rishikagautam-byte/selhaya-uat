...
export default function TheHouseAndCollectionsPage() {
    const { articles, loading } = useJournalArticles("the-house-and-collections");
    ...
    return (
        <>
            <SEO
                title={SEO_CONFIG.theHouseAndCollections.title}
                description={SEO_CONFIG.theHouseAndCollections.description}
                canonical={SEO_CONFIG.theHouseAndCollections.canonical}
                type="article"
            />
            <HeroSection
            ...
            <section className="bg-primary-light py-10 px-6 md:px-12 lg:px-20">
                {loading ? (
                    ...
                    >
                        <p className="text-[14px] uppercase tracking-widest text-text/50 mb-4">The House &amp; Collections</p>
                        ...
                        >
                            <Link to={item.href} className="overflow-hidden block">
                                <img
                                    src={item.image}
                                    alt={item.title}
                                    className="aspect-square w-full h-auto object-cover transition-transform duration-700 group-hover:scale-105"
                                />
                            </Link>
                            <Link
                                to={item.href}
