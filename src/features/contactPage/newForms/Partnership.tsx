...
import partnershipImg from "../../../assets/forms/partnership.png";
...
const PARTNERSHIP_OPTIONS_COL1 = [
  "Luxury / Brand Partnership",
  "Cultural Partnership",
  "Philanthropic / Cultural Initiative",
];

const PARTNERSHIP_OPTIONS_COL2 = [
  "Event / Experience",
  "Others",
];
...
const Partnership = () => {
  ...
  const handlePartnershipToggle = (option: string) => {
  ...
  const validateForm = () => {
    ...
    if (selectedPartnerships.length === 0) {
      toast.error("Please select at least one partnership type");
      return false;
    }
    if (!form.enquiry.trim()) {
      toast.error("Please tell us about your enquiry");
      return false;
    }
  ...
  const submitEnquiry = async (token: string) => {
    ...
    try {
      ...
      formData.append("action", "PartnershipEnquiry");
      formData.append("fullName", form.fullName.trim());
      formData.append("email", form.email.trim());
      formData.append("organisation", form.organisation.trim());
      formData.append("location", selectedLocation?.name || "");
      formData.append("partnershipType", selectedPartnerships.join(", "));
      formData.append("message", form.enquiry.trim());
  ...
  return (
    <>
      <SEO
        title="Begin a Partnership Conversation | SELHAYA®"
        description="SELHAYA works with selected luxury partners, cultural institutions, patrons and organisations to create meaningful collaborations and cultural initiatives."
      ...
      <section className="min-h-screen grid grid-cols-1 lg:grid-cols-[1.5fr_1fr] bg-primary-light text-text">
        ...
        <div className="w-full flex flex-col justify-center px-6 xl:px-16 py-12 md:py-6  mt-20 overflow-y-auto">
          <div className="lg:pr-20">
            ...
            <p className="text-[16px] text-text/90 mb-8 ">
              SELHAYA works with selected luxury partners, cultural institutions, patrons and organisations to create meaningful collaborations and cultural initiatives.
            </p>

            {/* Form Section */}
            <div className="w-full flex flex-col">
              <p className="dm-sans text-[14px] sm:text-[16px] uppercase  font-semibold text-text mb-4">
                Enquiry Details
              </p>
              ...
              {/* Partnership Type Field Group */}
              <div className="mt-6">
                <label className="text-[14px] text-[#281B1380] block mb-3 font-normal">
                  Partnership Type*
                </label>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-2.5">
                  ...
                  <div className="flex flex-col gap-2.5">
                    {PARTNERSHIP_OPTIONS_COL1.map((option) => {
                      const isChecked = selectedPartnerships.includes(option);
                      return (
                        <label
                          key={option}
                        ...
                        >
                          <div className="relative flex items-center justify-center shrink-0">
                            <input
                              type="checkbox"
                              checked={isChecked}
                              onChange={() => handlePartnershipToggle(option)}
                          ...
                          <span className="text-[13px] sm:text-[14px] text-[#281B1380] group-hover:text-[#281B13] transition-colors font-light">
                            {option}
                          </span>
                        </label>
                      );
                  ...
                  {/* Column 2 */}
                  <div className="flex flex-col gap-2.5">
                    {PARTNERSHIP_OPTIONS_COL2.map((option) => {
                      const isChecked = selectedPartnerships.includes(option);
                      return (
                        <label
                          key={option}
              ...
              <div className="flex items-start gap-3 mt-6">
                ...
                <p className="text-[12px] leading-relaxed text-[#281B1380]">
                  I agree to be contacted by SELHAYA regarding my enquiry and accept the{" "}
                  <Link
                    to="/policies/privacy"
                    className="underline hover:text-[#281B13] transition-colors"
                  >
                    Privacy Policy
                  </Link>
                  .*
              ...
              <div className="flex items-start gap-3 mt-3">
                ...
                <p className="text-[12px] leading-relaxed text-text">
                  I agree to receive marketing and promotional emails from SELHAYA.
                </p>
              </div>
              ...
              {/* reCAPTCHA — Visible after clicking submit or always ready */}
              {showCaptcha && (
                <div className="mt-5">
                  <ReCAPTCHA
                    ref={captchaRef}
        ...
        {/* RIGHT PANEL — Image (Hidden for md screens and lesser, visible on lg and up) */}
        <div className="hidden lg:block relative w-full h-[100dvh] sticky top-0 overflow-hidden">
          <img
            src={partnershipImg}
            alt="SELHAYA Partnership"
